...
def log(file_name, text,path=None, act='a'):
    if path is None:
      path = ''
...
for test in range(2,num_tests):
  ...
  agent_name = 'TD3_SMC_Agent_'+str(test)
  ...
  # log file
  if log_file:
    log(agent_name,get_timedate()+' Start!',path=path, act='w')
    header = 'Date '\
             'Time '\
             'Episode '\
             'Position '\
  ...
  for i in range(epochs):
      ...
      while not terminal:
          ...
          gains = [abs(adj_action[0]), abs(adj_action[1]), abs(adj_action[2])]             
          
          # applying adjutment action
          ...
          if len(memory) <= 1000:
              adj_action = [control.alpha[0],control.rho[0],control.K[0]]
      ...
      plt.plot(curve)
      plt.grid()
      fig.suptitle('Reward - '+agent_name, fontsize=14)
