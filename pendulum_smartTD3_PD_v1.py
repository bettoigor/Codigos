...
control = PDControl(K_p=kp, K_d=kd)

# Initializing RL objects
num_states = 3
num_actions = 2
memory = Memory(num_states,num_actions)
agent_name = 'Agent_1_SmartPD_1'
...
for i in range(epochs):
    ...
    while not terminal:
        ...
        print(' ********** Training **************',
              ...
              '\n * K_p:',round(control.K_p,5),
              '\n * K_d:',round(control.K_d,5),          
    ...
    plt.plot(curve)
    plt.grid()
    fig.suptitle('Received Reward - SmartPD'+agent_name, fontsize=16)
...
plt.xlabel('Epochs', fontsize=14)
plt.ylabel('Reward', fontsize=14)
plt.savefig('images/PD_out_'+agent_name+'.png')
plt.show(block=True)
